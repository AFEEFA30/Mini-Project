import React from 'react'
import './userData.css'
function UserData() {
  return (
    <div className='adminCollection'>
        <h1>User List</h1>

        <table className='collectionListTable'>
          <thead>
            <tr>
              <th>Items</th>
              <th>Title</th>
              <th>Collection</th>
              <th>Description</th>
              <th>Price</th>
              <th>Action</th>
            </tr>
          </thead>
          
          <tbody>
          <tr>
            <td>1</td>
            <td>Churidar</td>
            <td><img src="https://th.bing.com/th/id/OIP.sviybmjtbUbEY9xu3h4QCQHaLH?w=179&h=268&c=7&r=0&o=5&pid=1.7"></img></td>
            <td>Churidar for women </td>
            <td>$1250</td>
            <td><button className='collection-btn'>Delete</button></td>
          </tr>
          <tr>
            <td>2</td>
            <td>Saree</td>
            <td><img src="https://th.bing.com/th/id/OIP.8JhmN5FlB__BiCWd2W-m9gHaJ2?w=179&h=239&c=7&r=0&o=5&pid=1.7"></img></td>
            <td>Indian Wedding Wear Saree </td>
            <td>$3420</td>
            <td><button className='collection-btn'>Delete</button></td>
          </tr>
          <tr>
            <td>3</td>
            <td>Monochrome Style</td>
            <td><img src=" https://th.bing.com/th/id/OIP.KEhLtgGMWIdlbVaW08_jOAHaNK?w=179&h=319&c=7&r=0&o=5&pid=1.7"></img></td>
            <td>Monochrome Dressing Style for men</td>
            <td>$1260</td>
            <td><button className='collection-btn'>Delete</button></td>
          </tr>
          <tr>
            <td>4</td>
            <td>Style in Winter</td>
            <td><img src="https://th.bing.com/th/id/OIP.In-P9MRuXvv5IHDMK4GhdgHaK1?w=179&h=262&c=7&r=0&o=5&pid=1.7 "></img></td>
            <td>Dressing Style for Men in Winter </td>
            <td>$350</td>
            <td><button className='collection-btn'>Delete</button></td>
          </tr>
          <tr>
            <td>5</td>
            <td>Frock</td>
            <td><img src= "data:image/jpeg;base64,"></img></td>
            <td>Long Frock for kid </td>
            <td>$2250</td>
            <td><button className='collection-btn'>Delete</button></td>
          </tr>
          <tr>
            <td>6</td>
            <td>Summer Dress</td>
            <td><img src="https://th.bing.com/th/id/OIP.rXUUG8QNor00i4NSoeNXVwHaHY?w=200&h=199&c=7&r=0&o=5&pid=1.7"></img></td>
            <td>Summer Dress for Kid </td>
            <td>$250</td>
            <td><button className='collection-btn'>Delete</button></td>
          </tr>
          </tbody>
          </table>
          </div>
  )
}

export default UserData;